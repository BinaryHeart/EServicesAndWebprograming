...
Målet med detta steg är att skapa en sida som innehåller ett formulär. Detta formulär ska innehålla alla fält som användaren behöver för att kunna registrera sig. Alltså skapa en användare.
  ...
  Nu skall vi se till inloggningen faktiskt spelar någon roll. Målet är att ``skydda'' index-sidan. Alltså sidan där man kan posta blogginlägg som ni skapade i laboration 1.

  Endast användare som är inloggade skall kunna se sidan. Om man skulle skriva in adressen till sidan i webbläsarens adressfält, utan att vara inloggad, skall användaren antingen (A) motta ett meddelande om att de måste logga in, eller (B) förflyttas (\texttt{redirect}) till inloggningssidan automatiskt. 
